 ...
 * during a Breadth First Search of Nearby words to create a path
 * between two words.
 * 
...
public class WPTree implements WordPath {
	...
	public WPTree() {
		this.root = null;
		// TODO DONE - initialize a NearbyWords object
	...
	public List<String> findPath(String word1, String word2) {
		...
		while (!queue.isEmpty()) {
			...
			final List<String> neighboors = nw.distanceOne(currNode.getWord(),
			        true);

			for (String neighboor : neighboors) {
				if (!visited.contains(neighboor)) {
					visited.add(neighboor);
					WPTreeNode childNode = currNode.addChild(neighboor);
					queue.add(childNode);
					if (neighboor.equals(word2)) {
 ...
 * contain a word in the dictionary and the relationship between nodes
 * is that a child is one character mutation (deletion, insertion, or
...
class WPTreeNode {
	...
	/**
	 * Construct a node with the word w and the parent p (pass a null
	 * parent to construct the root)
	 * 
	 * @param w
	 *            The new node's word
	 * @param p
	...
	/**
	 * Add a child of a node containing the String s precondition: The
	 * word is not already a child of this node
	 * 
	 * @param s
	...
	/**
	 * Get the list of children of the calling object (pass a null
	 * parent to construct the root)
	 * 
	...
	/**
	 * Allows you to build a path from the root node to the calling
	 * object
	 * 
	 * @return The list of strings starting at the root and ending at
	 *         the calling object
	 */
